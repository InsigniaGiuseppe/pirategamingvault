...
const AdminVideoManager = () => {
  ...
  const processBulkVideos = async () => {
    ...
    try {
      ...
      const shortsAdded = savedVideos.filter(v => v.tags?.includes('shorts')).length;
  ...
  const updateVideoReward = async (video: Video, newReward: number) => {
    try {
      const updated = await updateVideo(video.id, { coin_reward: newReward });
      setVideos(prev => prev.map(v => v.id === video.id ? updated : v));
  ...
  const exportVideos = () => {
    const csvContent = [
      ...
      ...videos.map(video => [
        ...
        video.is_active,
        video.view_count,
        'N/A', // completion_count not available
        'N/A'  // original_url not available
      ].join(','))
    ].join('\n');
  ...
  const totalRewards = videos.reduce((sum, v) => sum + v.coin_reward, 0);
                        ...
                        <TableRow key={video.id}>
                          ...
                          <TableCell>
                            <div className="text-sm">
                              ...
                              <div>Featured: {video.is_featured ? 'Yes' : 'No'}</div>
                            </div>
                          </TableCell>
                          <TableCell>
